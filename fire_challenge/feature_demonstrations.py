...
def compare_strategies(map_num=0):
    """
    Demonstrate how to easily test multiple strategies with the API.
...
def demonstrate_reset(map_num=0):
    ...
    # Find some open cells for testing
    grid = game.grid
    open_cells = [(x, y) for y in range(grid.shape[0]) 
                  for x in range(grid.shape[1]) if grid[y, x] == 0]
    
    # Respect the max_walls limit
    walls_for_demo = min(game.max_walls, len(open_cells))
    if walls_for_demo < 1:
        print("Not enough open cells or wall budget to demonstrate reset functionality")
        return None, None
    
    # Calculate how many walls to use per strategy (split the budget)
    walls_per_strategy = max(1, walls_for_demo // 2)
    
    # Try first strategy
    strategy1_walls = open_cells[:walls_per_strategy]
    print(f"Strategy 1: Place {len(strategy1_walls)} wall(s) at {strategy1_walls}")
    game.place_walls(strategy1_walls)
    ...
    # Try second strategy with different open cells
    strategy2_walls = open_cells[walls_per_strategy:walls_per_strategy * 2]
    if len(strategy2_walls) > game.max_walls:
        strategy2_walls = strategy2_walls[:game.max_walls]
    
    print(f"Strategy 2: Place {len(strategy2_walls)} wall(s) at {strategy2_walls}")
    game.place_walls(strategy2_walls)
...
def demonstrate_properties(map_num=0):
    ...
    # Find some open cells for placing walls
    grid = game.grid
    open_cells = [(x, y) for y in range(grid.shape[0]) 
                  for x in range(grid.shape[1]) if grid[y, x] == 0]
    
    # Respect max_walls limit
    num_walls_to_place = min(game.max_walls, len(open_cells))
    
    if num_walls_to_place >= 1:
        # Place some walls and show updated properties
        walls_to_place = open_cells[:num_walls_to_place]
        print(f"\nPlacing {num_walls_to_place} wall(s) at {walls_to_place}...")
        ...
        print("\nNot enough open cells or wall budget to demonstrate wall placement")


if __name__ == "__main__":
    print("Fire Challenge - Examples")
    print("=" * 60)
