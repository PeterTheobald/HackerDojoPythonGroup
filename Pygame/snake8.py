...
pygame.display.set_caption("Snake with Tail, Food Image, and Score")
...
food_image = pygame.transform.scale(food_image, (rect_size, rect_size))
...
# Pick a starting position (aligned) near the bottom center
start_x = valid_x_positions[len(valid_x_positions) // 2]
start_y = valid_y_positions[-2]

# Initial snake of length 4 (head + 3-tail), vertically aligned
snake_segments = [
    (start_x, start_y),
...
for seg_x, seg_y in snake_segments:
    pygame.draw.rect(screen, SNAKE_COLOR, (seg_x, seg_y, rect_size, rect_size), border_radius=5)

# Draw initial food image
...
while running:
    ...
    if not game_over:
        ...
        # Only update tail if it was popped
